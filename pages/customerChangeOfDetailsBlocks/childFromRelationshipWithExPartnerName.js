var childFromRelationshipWithExPartnerNameCommands = {
  submit: function() {
    this.api.pause(1000);
    return this.waitForElementVisible('@body', 1000)
    	.assert.elementPresent('@title')
    	.assert.elementPresent('@givenName')
    	.assert.elementPresent('@familyName')
    	.click('select[id="titleRef"] option[value="Mr"]')
			.setValue('@givenName', 'TEST')
			.setValue('@familyName', 'Child')
...
module.exports = {
  commands: [childFromRelationshipWithExPartnerNameCommands],
	elements: {
		body: 'body',
		title: {
